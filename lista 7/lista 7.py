...
# Kodowanie zmiennych kategorycznych
credit_card_data = przetwarzanie_danych(credit_card_data)

res, feat = zbuduj_modele_i_wykonaj_ocene(credit_card_data, target='LIMIT_BAL')

# Wyświetlenie najważniejszych cech dla każdego modelu
for model_name, feature_importance in feat.items():
    if feature_importance is not None:
        sorted_features = sorted(feature_importance.items(), key=lambda x: x[1], reverse=True)
        print(f"\nNajważniejsze cechy dla modelu {model_name}:")
        for feature, importance in sorted_features[:5]:  # Top 5 cech
